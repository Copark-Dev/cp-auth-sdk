...
export class CpAuthClient {
    ...
    async ensureAccessToken() {
        ...
        if (!t)
            return null;
        if (exp && Date.now() < exp - REFRESH_SKEW_MS)
            return t;
    ...
    async refresh() {
        ...
        this.refreshInflight = (async () => {
            try {
                const tr = await this.post("/auth/refresh", {
                    refresh_token: rt,
                });
    ...
    async login(email, password) {
        const tr = await this.post("/auth/login", {
            email,
            password,
        });
        this.storage.set(tr.access_token, tr.refresh_token, tr.expires_in);
    ...
    async signup(args) {
        const raw = await this.post("/auth/signup", args);
        if (raw.access_token) {
            this.storage.set(raw.access_token, raw.refresh_token, raw.expires_in ?? 3600);
            return {
    ...
    async getMe() {
        ...
        if (!t)
            throw new CpAuthApiError(401, "Not authenticated");
    ...
    async verifyEmail(token, type = "signup") {
        const tr = await this.post("/auth/email-verify", {
            token,
            type,
        });
        this.storage.set(tr.access_token, tr.refresh_token, tr.expires_in);
     ...
     */
    async startOAuth(provider, redirectPathOrOpts = "/auth/callback") {
        const opts = typeof redirectPathOrOpts === "string"
        ...
        const r = await fetch(`${this.authUrl}/auth/oauth/url?provider=${provider}&redirect_url=${encodeURIComponent(callbackUrl)}`, { method: "POST" });
        if (!r.ok)
            throw await this.toError(r);
    ...
    buildChromeIntent(href) {
        const resolved = href ?? (typeof window !== "undefined" ? window.location.href : "");
        if (!resolved) {
            throw new Error("buildChromeIntent: no href available — pass one explicitly when running outside a browser (React Native, Node, etc.)");
        }
